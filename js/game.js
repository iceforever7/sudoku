...
plygame().then(() => {
    ...
    const containers = document.querySelectorAll('div.content');
...
var sBoards=new Array();

function solveBoard(i) {
    let ccc="solve_container"
    tu.stop();
    let tBoard;
    tBoard = document.getElementById("board");
    tBoard = document.createElement("table");
    ...
    for(let i = 0; i < 9; i++) {
        ...
        for(let j = 0; j < 9; j++) {
            td = document.createElement("td");
            value = h[i][j];
            td.setAttribute("class", "solve-show");
            td.innerHTML = value
            tr.appendChild(td);
        }
        tBoard.appendChild(tr);
